...
- **Local RAG Search Platform** — Source: verified repo audit (provided 2026-07-28). Headline: provenance headers baked into chunk text so citations survive any retrieval path; fully air-gapped operation right-sized to a 1B model with CPU fallback; candid analysis of hybrid retrieval's failure modes (substring-tie collapse as the case for real BM25/IDF, O(corpus) keyword scans, source-level dedup vs. a one-big-document corpus). **Hard constraints:** only reproducible numbers — keyword baseline 34% @k=3 / 50% @k=5, 3,796 chunks, 50MB index, 9 sources, ~700 Python LOC. Never use the unverifiable resume/README numbers (2%→66%, 40%→80%, sub-100ms TTFT). Describe transport accurately as a streaming-fetch token UI (not SSE-framed). Frame limitations as identified next steps.
