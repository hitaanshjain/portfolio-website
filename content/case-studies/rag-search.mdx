...
and being honest about the answer.
...
- **Publish the ugly baseline.** I would rather show the diagnostic number and
  its failure modes than quote one I can't reproduce.

## Results & next steps

It works as a single-user prototype, and every answer carries verified
provenance. Next, in order: BM25/IDF to fix tie collapse, passage-level ground
truth so the benchmark measures relevance rather than attribution, and
first-token latency measurement for the streaming path.
